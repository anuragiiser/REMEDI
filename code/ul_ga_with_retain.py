...
#         num_training_steps=len(forget_loader) * EPOCHS
#     )
...
for UNLEARN_K in forget_details:
    ...
    for fold in forget_details[UNLEARN_K]:
        ...
        sampled_retain_loader = DataLoader(
            ...
            batch_size=RETAIN_PER_FORGET*BATCH_SIZE,
            shuffle=True,
        )
        ...
        for epoch in range(EPOCHS):
            ...
            # progress_bar = tqdm(enumerate(zip(forget_loader, retain_loader)), desc=f'Epoch {epoch + 1}/{EPOCHS}')
            for i, (forget_batch, retain_batch) in enumerate(zip(forget_loader, sampled_retain_loader)):
                ...
                optimizer.step()
                # scheduler.step()
                batch_len = len(forget_batch)
                train_loss += retain_loss.item()*batch_len
                total_forget_loss += forget_loss.item()*batch_len
                samples += batch_len
                # progress_bar.set_postfix({
                #     'retain_loss': retain_loss.item(),
                #     'forget_loss': forget_loss.item(),
                # })
                print(f"Batch {i+1}/{total_batches} Retain loss: {retain_loss.item()} Forget loss: {forget_loss.item()}")
            
            avg_train_loss = train_loss/samples  
            avg_forget_loss = total_forget_loss/samples
            
            print(f"\nEpoch {epoch + 1} Avg Train Loss: {avg_train_loss:.4f} Avg Forget Loss: {avg_forget_loss: .4f}")
            forget_metrics = test(model, forget_loader, device)
