...
UNLEARNED_MODEL_PATH = "unlearned/"+MODEL+"/ga_without_retain/"
FORGET_DETAILS_PATH = args.forget_details_path

start_time = datetime.now()

# UNLEARNED_MODEL_PATH = "./unlearned/ga_without_retain/"
LAMBDA = 0.4
    ...
    for fold in forget_details[UNLEARN_K]:
        ...
        for epoch in range(EPOCHS):
            ...
            for i, forget_batch in enumerate(forget_loader):
                ...
                # })
                print(f"Batch {i+1}/{total_batches} Forget loss: {forget_loss.item()}")
            
            avg_forget_loss = total_forget_loss/samples
            
            print(f"\nEpoch {epoch + 1} Avg Forget Loss: {avg_forget_loss: .4f}")
            forget_metrics = test(model, forget_loader, device)
