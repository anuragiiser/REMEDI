...
for UNLEARN_K in forget_details:
    ...
    for fold in forget_details[UNLEARN_K]:
        ...
        importance = estimate_parameter_importance(forget_loader, model, device)
        ...
        for epoch in range(EPOCHS):
            ...
            for i, (forget_batch, adv_batch) in enumerate(zip(forget_loader, adv_loader)):
                ...
                reg_loss = parameter_regularization_loss(model, origin_params, importance)
                ...
                # })
                print(f"Batch {i+1}/{total_batches} Adv loss: {adv_loss.item()} Forget loss: {forget_loss.item()} Reg Loss: {reg_loss.item()}")
